<a href="{{ route('games.show', $game->slug) }}" class="block bg-gray-800 rounded-lg overflow-hidden hover:ring-2 hover:ring-blue-500 transition-all duration-300 group">
    ...
    <div class="p-4">
        ...
        <p class="text-gray-500 text-xs mt-1">{{ $game->developer }}</p>
        <div class="flex items-center justify-between mt-3">
            <div>
                @if($game->is_discounted)
                    <span class="text-gray-500 line-through text-sm">${{ number_format($game->price, 2) }}</span>
            ...
            @if($game->genres->count())
                <span class="text-xs text-gray-500 bg-gray-700 px-2 py-1 rounded">{{ $game->genres->first()->name }}</span>
            @endif
        </div>
    </div>
</a>
