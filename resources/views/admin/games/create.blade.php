<x-app-layout>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form method="POST" action="{{ route('admin.games.store') }}" class="bg-gray-800 rounded-lg p-8 space-y-6">
            @csrf
                ...
                <div>
                    ...
                    <select id="genres" name="genres[]" multiple
                        ...
                        @foreach($genres as $genre)
                            <option value="{{ $genre->id }}" @selected(in_array($genre->id, old('genres', [])))>{{ $genre->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="6" required
                          class="block w-full rounded-md border-gray-600 bg-gray-900 text-white focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('description') }}</textarea>
            </div>
