        ...
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    @foreach($cartItems as $item)
                        <div class="flex items-center bg-gray-800 rounded-lg p-4">
                            ...
                            <div class="flex-1 ml-4">
                                ...
                                <p class="text-gray-400 text-sm mt-1">{{ $item->game->developer }}</p>
                                <div class="flex justify-between items-center mt-3">
                                    <div class="flex items-center">
                                        @if($item->game->is_discounted)
                                            <span class="text-gray-400 line-through text-sm">${{ number_format($item->game->price, 2) }}</span>
                                    ...
                                    <form method="POST" action="{{ route('cart.remove', $item) }}" onsubmit="return confirm('Remove this game from cart?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-gray-400 hover:text-red-400 text-sm transition">Remove</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-800 rounded-lg p-6 h-fit sticky top-4">
                    ...
                    <div class="mt-4">
                        <a href="{{ route('games.index') }}" class="text-center block text-sm text-gray-300 hover:text-white transition">Continue Shopping</a>
                    </div>
                </div>
            </div>
        @endif
